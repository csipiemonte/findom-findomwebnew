 ...
 * @author Phil Steitz
 * @since 1.0
 * @version $Id: StringUtils.java 1.1 2005/06/28 08:31:28 cambrosio Exp $
 */
public class StringUtils {
  ...
  // (not sure who tested this)
	...
	.getLogger(Constants.APPLICATION_CODE + ".util");
  ...
  public static String join(Object[] array, char separator) {
    ...
    int bufSize = (arraySize == 0 ? 0 :
                   ( (array[0] == null ? 16 : array[0].toString().length()) + 1) *
                   arraySize);
  ...
  public static Vector parseKeywords(String keywordString) {
    if (keywordString != null) {
      ...
      try {
        ...
        log.error("Error while parsing search string", e);
      }
    }
    return null;
  }
  
	public static int getGiorniPeriodo(String dataInizioPeriodo, String dataFinePeriodo) {
		int days = 0;
		try {
			...
			Date d1 = fmt.parse(dataInizioPeriodo);
			Date d2 = fmt.parse(dataFinePeriodo);
			...
			days = (int) (millisDiff / 86400000);
			 ...
			 log.error("Error in getGiorniPeriodo: ", e);
		}
		return days;
	}
