...
public class CustomDate {
	...
	public String format(String year, String month, String day, int formatType) {
		...
		try {
			...
			case DD_MMMM_YY:
				return day + " " + monthNames.elementAt(monthValue) + " " + year.substring(2, 4);
			case DD_MMM_YYYY:
				return day + " " + ((String) monthNames.elementAt(monthValue)).substring(0, 3) + " " + year;
			case DD_MMM_YY:
				return day + " " + ((String) monthNames.elementAt(monthValue)).substring(0, 3) + " " + year.substring(2, 4);
			case YYYY_MM_DD:
			...
			log.error("Exception in format from year/month/day : ", e);
		}
		return null;
	}
	...
	/*
	 * Return a 2 characters long string containing the month. If the given value
	 * month is not proper it returns null.
	 */
